// ============================================
// ARCHTEN - Navigation & Settings Manager
// ============================================
...
class NavigationSystemManager {
    constructor() {
        ...
        this.navMobileClose = document.getElementById('navMobileClose');
        this.navMobileOverlay = document.querySelector('.nav-mobile-overlay');
        this.navMobileLinks = document.querySelectorAll('.nav-mobile-section a');
        this.navLinks = document.querySelectorAll('.nav-link');

        if (!this.hamburgerBtn || !this.navMobileMenu) {
    ...
    attachEventListeners() {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && this.navMobileMenu.classList.contains('active')) {
                this.closeMenu();
            }
        });
    ...
    closeMenu() {
        const panel = this.navMobileMenu.querySelector('.nav-mobile-panel');
        if (panel) {
            panel.style.animation = 'slideOutLeft 0.3s ease-out forwards';
        }
        setTimeout(() => {
...
class HeaderSettingsManager {
    ...
    saveSettings() {
        ...
        if (window.setLanguage) {
            window.setLanguage(this.currentSettings.language);
        }
    ...
    updateUI() {
        if (this.language) this.language.value = this.currentSettings.language || 'id';
        if (this.volume) this.volume.value = this.currentSettings.volume || 70;
        if (this.volumeValue) this.volumeValue.textContent = (this.currentSettings.volume || 70) + '%';
        if (this.soundEnabled) this.soundEnabled.checked = this.currentSettings.soundEnabled !== false;
        if (this.notificationEnabled) this.notificationEnabled.checked = this.currentSettings.notificationEnabled !== false;
        if (this.textSize) this.textSize.value = this.currentSettings.textSize || 'normal';
        if (this.analytics) this.analytics.checked = this.currentSettings.analytics !== false;
        if (this.cookies) this.cookies.checked = this.currentSettings.cookies !== false;
    }

    applySettings() {
        this.applyTextSize(this.currentSettings.textSize || 'normal');
    }
    ...
    showNotification(message, type = 'info') {
        const colors = {
            success: 'linear-gradient(135deg, #7c3aed, #06b6d4)',
            info: 'linear-gradient(135deg, #06b6d4, #7c3aed)',
            error: 'linear-gradient(135deg, #f43f5e, #7c3aed)'
        };
        ...
        setTimeout(() => {
            notification.style.animation = 'slideOutToRight 0.4s cubic-bezier(0.4, 0, 0.2, 1) forwards';
            setTimeout(() => {
                notification.remove();
                style.remove();
            }, 400);
...
function openFeatures(e) {
            e.preventDefault();
            document.getElementById('fitur').classList.add('active');
            document.body.style.overflow = 'hidden';
        }
        
function closeFeatures() {
            document.getElementById('fitur').classList.remove('active');
            document.body.style.overflow = '';
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    new NavigationSystemManager();
    console.log('🌑 ARCHTEN Dark Mode — System Initialized');
});
