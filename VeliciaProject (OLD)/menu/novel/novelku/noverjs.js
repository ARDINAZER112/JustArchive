...
home: () => `
        <div class="mb-8 md:mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            ${currentUser ? `
            <button onclick="clearSearchAndGoHome('upload')" class="animate-fade-in-up delay-100 w-full md:w-auto justify-center bg-indigo-600 dark:bg-indigo-500 text-white px-8 py-4 rounded-2xl font-bold hover:shadow-xl hover:shadow-indigo-200 dark:hover:shadow-indigo-900/50 transition-all flex items-center gap-2 flex-shrink-0">
                ...
                <div class="max-w-md w-full mx-auto relative pt-4 md:pt-0">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            <!---<button onclick="handleOAuth('facebook')" class="flex items-center justify-center gap-2 w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-sm active:scale-95">
                            ...
                            <!---<button onclick="handleOAuth('apple')" class="flex items-center justify-center gap-2 w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-sm active:scale-95">
...
forgot: () => `
        <div class="fixed inset-0 z-50 flex flex-col md:flex-row bg-white dark:bg-slate-950 overflow-y-auto md:overflow-hidden transition-colors duration-300">
            ...
            <button onclick="showView('auth')" class="fixed top-4 right-4 md:top-8 md:right-8 z- w-11 h-11 flex items-center justify-center bg-slate-900/40 hover:bg-slate-900/60 md:bg-slate-100 md:hover:bg-slate-200 md:dark:bg-slate-800 md:dark:hover:bg-slate-700 text-white md:text-slate-600 md:dark:text-slate-300 backdrop-blur-md rounded-2xl transition-all shadow-lg border border-white/20 md:border-none">
                <svg width="24" height="24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
            </button>
        </div>
    `,
    upload: () => `
        <div class="max-w-6xl mx-auto animate-fade-in-up">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">Terbitkan <span class="text-indigo-600 dark:text-indigo-400">Naskah</span></h2>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button id="btn-upload" onclick="handleUpload()" class="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl shadow-indigo-200/50 dark:shadow-indigo-900/30 hover:bg-indigo-700 transition-all text-sm">DITERBITKAN</button>
                </div>
            </div>
...
function renderNovelGrid(query = '') {
    ...
    // FIX DRAFT BOCOR: Pastikan draft hanya tampil bagi pemiliknya
...
async function handleUpload() {
    ...
    try {
        ...
        const { error } = await _supabase.from('novels').insert([{
            ...
            status: 'published' // FIX: Pastikan yang diupload ter-set ke published
        }]);
...
async function handleOAuth(provider) {
    ...
    try {
        ...
        showToast("Gagal terhubung dengan " + provider + ": " + e.message, "error");
    }
}
...
async function handleLogin() {
    ...
    const rememberMe = document.getElementById('remember-me'); // Ambil elemen checkbox
...
async function handleAvatarUpload(event) {
    const files = event.target.files;
    // FIX AVATAR UPLOAD: Pastikan array ada dan tidak kosong, lalu ambil indeks 0
...
window.onload = async () => {
    if (initSupabase()) {
        ...
        // FIX SINKRONISASI OAUTH: Deteksi redirect setelah login OAuth berhasil
